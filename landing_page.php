 ...
 */
function obtenerImagenProducto($nombre, $tipo) {
    // Clave de acceso de Unsplash
    $UNSPLASH_ACCESS_KEY = 'Y2fTCP-qwJ10mlekoYOrgt5oMdrKjb1W7Uf5cWg02os';
    $query = urlencode(strtolower("$tipo $nombre"));
    $url = "https://api.unsplash.com/search/photos?query=$query&client_id=$UNSPLASH_ACCESS_KEY&per_page=1";
    ...
    try {
        ...
        error_log("Error al obtener imagen de Unsplash para producto '$nombre': " . $e->getMessage());
    }

    // Genera un placeholder si la API falla
    $placeholder_text = substr($nombre, 0, 2);
...
try {
    $stmt_produccion = $conn->query("SELECT * FROM inventario WHERE tipo = 'produccion' ORDER BY fecha_ingreso DESC LIMIT 8");
    if ($stmt_produccion) {
        $productos_produccion = $stmt_produccion->fetchAll(PDO::FETCH_ASSOC);
    }
} catch (PDOException $e) {
    error_log("Error al cargar productos de producción: " . $e->getMessage());
}
...
try {
    ...
    $total_animales = $result_total_animales['total'];
      ...
      $stmt_total_usuarios = $conn->prepare("SELECT COUNT(id) AS total FROM usuarios WHERE rol IN ('administrador', 'veterinario', 'trabajador')");
    ...
    // Obtener el total de cantidad de productos en la categoría producción
$stmt_total_produccion = $conn->prepare("SELECT SUM(cantidad) AS total FROM inventario WHERE tipo = 'produccion'");
$stmt_total_produccion->execute();
$result_total_produccion = $stmt_total_produccion->fetch(PDO::FETCH_ASSOC);
$total_produccion_registros = $result_total_produccion['total'] ?? 0;
} catch (PDOException $e) {
...
<body x-data="{
    ...
    <header class="bg-white dark:bg-gray-800 shadow-sm sticky top-0 z-40" x-data="{ open: false }">
        <div class="container mx-auto px-4 sm:px-6 lg:px-8 py-4 flex justify-between items-center">
            ...
            <div class="hidden md:flex items-center space-x-4">
                ...
                <a href="#production-products" class="nav-link font-medium transition-colors text-gray-600 dark:text-gray-300 hover:text-primary-600 dark:hover:text-primary-400">Produccion</a>
                ...
                <a href="./uploads/manual.pdf" class="nav-link font-medium transition-colors text-gray-600 dark:text-gray-300 hover:text-primary-600 dark:hover:text-primary-400">MANUAL DE USUARIO</a>
        ...
        <div x-show="open" x-cloak
            ...
            <div class="flex flex-col items-center space-y-3 pt-4 px-4">
                ...
                <a href="#production-products" @click="open = false" class="nav-link w-full text-center py-2 px-4 text-gray-600 dark:text-gray-300 hover:text-primary-600 dark:hover:text-primary-400 rounded-md">Produccion</a>
    ...
    <main class="flex-grow">
        ...
        <?php if ($usuario_logueado): ?>
            ...
            <section id="production-products" class="p-6 py-16 md:py-24 bg-gray-100 dark:bg-gray-900">
                <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                    <h2 class="text-3xl font-bold text-gray-900 dark:text-white text-center mb-10">Productos de Producción</h2>
                    <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                        <?php if (empty($productos_produccion)): ?>
                            <div class="col-span-full text-center p-8 bg-white dark:bg-gray-800 rounded-xl shadow-soft">
                                <p class="text-lg text-gray-500 dark:text-gray-400">No hay productos de producción disponibles actualmente.</p>
                            </div>
                        <?php else: ?>
                            <?php foreach ($productos_produccion as $producto): ?>
                                <?php
                                    // Cambia esta línea para usar la función obtenerImagenProducto()
                                    $image_url = obtenerImagenProducto($producto['nombre'], $producto['tipo'] ?? 'producto');
                                ?>
                                <div class="bg-white dark:bg-gray-800 rounded-xl shadow-soft overflow-hidden flex flex-col animal-card">
                                    <img src="<?= $image_url ?>" alt="<?= htmlspecialchars($producto['nombre']) ?>" class="w-full h-48 object-cover">
                                    <div class="p-5 flex-grow flex flex-col">
                                        <div class="flex items-center mb-2">
                                            <i class="fas fa-box text-2xl text-primary-600 mr-2"></i>
                                            <h3 class="text-xl font-bold text-gray-900 dark:text-white"><?= htmlspecialchars($producto['nombre']) ?></h3>
                                        </div>

                                        <div class="text-gray-600 dark:text-gray-400 text-sm mb-3 space-y-1">

                                            <p><span class="font-semibold flex items-center"><i class="bi bi-box-seam mr-2"></i>Cantidad:</span> <?= htmlspecialchars($producto['cantidad']) ?> <?= htmlspecialchars($producto['unidad'] ?? 'unidades') ?></p>
                                        </div>

                                        <div class="mt-auto pt-4 border-t border-gray-200 dark:border-gray-700">
                                            <p class="text-3xl font-extrabold text-primary-700 dark:text-primary-400 mb-4">$<?= number_format($producto['precio'], 2) ?></p>
                                            <button @click="openContactModal(<?= htmlspecialchars(json_encode(['nombre' => $producto['nombre'], 'descripcion' => $producto['descripcion']]), ENT_QUOTES, 'UTF-8') ?>, '<?= number_format($producto['precio'], 2) ?>')" class="w-full py-2 px-4 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors duration-200 flex items-center justify-center">
                                                <i class="bi bi-whatsapp mr-2"></i> Contactar
                                            </button>
                                        </div>
                                    </div>
        ...
        <?php else: ?>
            <section class="py-16 md:py-24 bg-gray-100 dark:bg-gray-900 text-center">
                <div class="container mx-auto px-4 sm:px-6 lg:px-8">
                    <div class="bg-white dark:bg-gray-800 rounded-xl shadow-soft-lg p-8 max-w-2xl mx-auto">
                        <i class="bi bi-lock-fill text-6xl text-primary-600 mb-6"></i>
                        <h2 class="text-3xl font-extrabold text-gray-900 dark:text-white mb-4">
                            Contenido Exclusivo para Usuarios Registrados
                        </h2>
                        <p class="text-lg text-gray-700 dark:text-gray-300 mb-6">
                            Para ver nuestras estadísticas de la granja, animales disponibles y productos de producción, por favor, inicia sesión o regístrate.
                        </p>
                        <div class="flex flex-col sm:flex-row justify-center space-y-4 sm:space-y-0 sm:space-x-4">
                            <a href="index.php" class="px-6 py-3 bg-primary-600 text-white rounded-lg hover:bg-primary-700 transition-colors duration-200 flex items-center justify-center">
                                <i class="bi bi-box-arrow-in-right mr-2"></i> Iniciar Sesión
                            </a>
                            <a href="register.php" class="px-6 py-3 bg-gray-200 text-gray-800 rounded-lg hover:bg-gray-300 dark:bg-gray-700 dark:text-gray-200 dark:hover:bg-gray-600 transition-colors duration-200 flex items-center justify-center">
                                <i class="bi bi-person-plus mr-2"></i> Registrar
                            </a>
                        </div>
                    </div>
            ...
            <div class="container mx-auto px-4 sm:px-6 lg:px-8 text-center">
                ...
                <div class="flex flex-col md:flex-row justify-center items-center space-y-6 md:space-y-0 md:space-x-8">
                    <div class="flex items-center text-gray-700 dark:text-gray-300">
                        ...
                        <span>Calle de la granja de rroron, Colombia</span>
                    </div>
                    <div class="flex items-center text-gray-700 dark:text-gray-300">
                        <i class="bi bi-envelope-fill text-primary-600 text-3xl mr-3"></i>
                        <a href="mailto:info@granjaapp.com" class="hover:underline">lagranjaderoron@granja.com</a>
                    </div>
                    <div class="flex items-center text-gray-700 dark:text-gray-300">
                        <i class="bi bi-phone-fill text-primary-600 text-3xl mr-3"></i>
                        <span>+57 314 804 2483</span>
                    </div>
                </div>
            </div>
        </section>
    ...
    <footer class="bg-white dark:bg-gray-800 border-t border-gray-200 dark:border-gray-700 py-6 px-4 sm:px-6 lg:px-8 mt-auto">
        <div class="container mx-auto flex flex-col md:flex-row justify-between items-center text-center md:text-left">
            <div class="text-sm text-gray-500 dark:text-gray-400 mb-2 md:mb-0">
                © <?= date('Y') ?> 
                LA GRANJA DE RORON  - Todos los derechos reservados.
            </div>
